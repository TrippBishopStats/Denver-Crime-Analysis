...
-- offense_category_id
SELECT COUNT(*) FROM crime_2013 WHERE is_traffic = FALSE;
-- this returns all records, so this column doesn't carry any information in it.
SELECT COUNT(*) FROM crime_2013 WHERE is_crime = TRUE;
-- this also returns all records, so this column doesn't carry any information in it either.
-- we can drop both columns.
ALTER TABLE crime_2013 DROP COLUMN is_traffic;
ALTER TABLE crime_2013 DROP COLUMN is_crime;
...
ALTER TABLE crime_2013 DROP COLUMN offense_type_id;
ALTER TABLE crime_2013 DROP COLUMN offense_category_id;

-- Now let's quickly look at some of the other columns to sanity check their data.
-- start with district_id
SELECT DISTINCT(district_id) FROM crime_2013;
-- interesting mix of values. There are blank values, a back tick, a "U", and "8" is missing.
SELECT district_id, COUNT(*) AS totals FROM crime_2013 GROUP BY district_id ORDER BY totals DESC;

-- update the empty strings to NULL
UPDATE crime_2013
SET district_id = NULL
WHERE district_id = '' OR district_id = '`';

-- explore victim_count
SELECT DISTINCT(victim_count) FROM crime_2013;
-- this data look good. No missing values and the value range is reasonable.
SELECT victim_count, COUNT(*) AS total FROM crime_2013 GROUP BY victim_count ORDER BY total DESC;
-- The vast majority of crimes involved a single victim, but the other numbers look reasonable. 27
-- victims in one incident is bananas!

-- explore precinct_id
SELECT DISTINCT(precinct_id) FROM crime_2013;
-- There are 40 precincts. Let's get a sense of the distribution of crime counts within the precincts
SELECT precinct_id, COUNT(*) AS total FROM crime_2013 GROUP BY precinct_id ORDER BY total DESC;
-- The data look reasonable.

-- explore neighborhood_id
SELECT DISTINCT(neighborhood_id) FROM crime_2013;
-- There are 78 neighborhoods and empty string. Let's get a sense of the distribution of crime counts 
-- within the neighborhoods
SELECT neighborhood_id, COUNT(*) AS total FROM crime_2013 GROUP BY neighborhood_id ORDER BY total DESC;
-- The data look reasonable, but 532 incidents don't have a neighborhood. These will be updated to NULL.
...
SELECT DISTINCT(offense_code) FROM crime_2013;
-- we have 137 of the 294 offense_codes represented in this table. Let's now look at extensions.
SELECT DISTINCT(offense_code_extension) FROM crime_2013;
-- codes 0-5 are represented in this data.
SELECT DISTINCT(offense_code, offense_code_extension) FROM crime_2013;
-- There are 178 distinct offense_code, offense_code_extension combinations
...
SELECT COUNT(*) FROM crime_2013 WHERE geo_x ISNULL OR geo_y ISNULL;
-- There are 711 incidents where one or both of the values is missing.
SELECT COUNT(*) FROM crime_2013 WHERE geo_lon ISNULL OR geo_lat ISNULL;
-- There are 711 incidents where one or both of the values is missing. Are they the same records?
SELECT COUNT(*) 
FROM crime_2013 
...
-- This returns a zero count, so all 711 records have an address. If would be possible to geocode them
...
SELECT COUNT(*) FROM crime_2013 WHERE first_occurrence_date ISNULL;
-- none, all records have a first_occurrence_date value.

-- How many incidents are missing last_occurrence_date?
SELECT COUNT(*) FROM crime_2013 WHERE last_occurrence_date ISNULL;
-- many, 21134 records are missing a last_occurrence_date value.

-- How many incidents are missing reported_date?
SELECT COUNT(*) FROM crime_2013 WHERE reported_date ISNULL;
-- none, all records have a reported_date value.

-- How often is the first_occurrence_date different to the reported_date?
SELECT COUNT(*) FROM crime_2013 WHERE first_occurrence_date <>reported_date;
-- quite often. In fact, this happens less than 50% of the time so we can't make assumptions
...
SELECT COUNT(*) FROM crime_2013 WHERE c_year = 2013;
-- year is good. All incidents happened in 2012.
-- Let's look at the incident counts by month.
SELECT c_month, COUNT(*) AS total FROM crime_2013 GROUP BY c_month ORDER BY c_month ASC;
-- month looks good. do the numbers add up?
SELECT SUM(total)
FROM (
	SELECT c_month, COUNT(*) AS total FROM crime_2013 GROUP BY c_month
) 
AS month_crime;
-- Yes.

-- Let's look at the incident counts by day.
SELECT c_day, COUNT(*) AS total FROM crime_2013 GROUP BY c_day ORDER BY c_day ASC;
-- day looks good. do the numbers add up?
SELECT SUM(total)
FROM (
	SELECT c_day, COUNT(*) AS total FROM crime_2013 GROUP BY c_day
) 
AS day_crime;
-- Yes.

-- Let's look at the incident counts by day of the week.
SELECT c_dow, COUNT(*) AS total FROM crime_2013 GROUP BY c_dow ORDER BY c_dow ASC;
-- day of week looks good. do the numbers add up?
SELECT SUM(total)
FROM (
	SELECT c_dow, COUNT(*) AS total FROM crime_2013 GROUP BY c_dow
) 
AS dow_crime;
-- Yes.